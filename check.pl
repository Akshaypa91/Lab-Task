#!/usr/bin/env perl
# use as: ~/src/checkpatch.pl --strict --max-line-length=60 -f fibonacci.c 
...
sub check_indentation {
    ...
    my @stack;      # Stack to track opening blocks
    my $line_num = 0;
    ...
    while (my $line = <$fh>) {
        ...
        # Count leading tabs
        #my $original_line = $line;  # Keep original for error reporting
        #my $leading_tabs = $original_line =~ s/^\t+//r;  # Remove leading tabs
        #my $tab_count = length($leading_tabs);

        my $original_line = $line;  # Keep original for debugging
        $original_line =~ s/^\t+//;          # Remove leading tabs for indentation check
        
        # Count the number of tabs for the current line
        my $tab_count = length($line) - length($original_line);

        #if ($line =~ /^\s*(\w[\w\s\*]*\s+)+\w+\s*\([^)]*\)\s*\{\s*$/) {
        if ($line =~ /^\s*(\w[\w\s\*]*\s+)+\w+\s*\([^)]*\)\s*$/) {
            push @stack, { type => 'function', line_num => $line_num, tab_count => $tab_count };
        }
        # Check for opening blocks with a brace on the same line
        if ($line =~ /^\s*(if|for|while|switch)\s*\(.*\)\s*\{\s*.*$/) {
            push @stack, { type => $1, line_num => $line_num, tab_count => $tab_count };
        } elsif ($line =~ /^\s*.*else\s*\{\s*.*$/ || $line =~ /^\s*.*else\s*if.*\{\s*.*$/) {
            if (@stack && $stack[-1]{type} eq 'if' || @stack && $stack[-1]{type} eq 'elsif') {
        		if($line =~ /^\s*.*else\s*\{\s*.*$/) {
                	push @stack, { type => 'else', line_num => $line_num, tab_count => $tab_count };
				}	
				else {
                	push @stack, { type => 'elsif', line_num => $line_num, tab_count => $tab_count };
                #print "else tabs= $tab_count\n";
            	} 
			} else {
                	print "INDENT: Unmatched 'else' at line $line_num\n";
					return 0;
           	}
        }
        ...
        if ($line =~ /^\s*\}/) {
            if (@stack) {
                my $block = pop @stack;
                if ($block->{tab_count} != $tab_count) {
                    print "INDENT: Error at line $line_num: '}' does not match block '$block->{type}' at line $block->{line_num} block.tab = $block->{tab_count} and tab_count = $tab_count\n";
					return 0;
                }
            } else {
                print "INDENT :Unmatched closing brace at line $line_num\n";
				return 0;
            }
        }
    }
    
    # Check for any unmatched opening blocks
    while (@stack) {
        my $block = pop @stack;
        print "INDENT: Unmatched block '$block->{type}' at line $block->{line_num}\n";
		return 0;
    }
...
EOM
	...
	$clean = check_indentation($filename);
	if ($quiet == 0) {
		print "\n";
		if ($clean == 1) {
